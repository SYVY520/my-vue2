  ...
  <div>
    ...
    <div  v-if="nr!=='1'" style="margin-left: 300px;margin-top: 100px;font-size: 40px;color: cadetblue">
      ...
      <course0 v-for="item in pageList" :key="item.id" >
        ...
        <a slot="homeworkImg">
          <router-link  v-if="identity==='teacher'" :to="{name:'tchomework',query: { course: item.coursename,status:0 }}"><img src="@/assets/2.png"></router-link>
          <router-link v-if="identity==='student'"  :to="{name:'homework1',query: { course: item.coursename,status:0 }}"><img src="@/assets/2.png"></router-link>
        </a>
        <a slot="homework">
          <router-link v-if="identity==='teacher'" :to="{name:'tchomework',query: { course: item.coursename,status:0 }}" style="text-decoration: none;color: #2a2a2a;">我的作业</router-link>
          <router-link v-if="identity==='student'" :to="{name:'homework1',query: { course: item.coursename,status:0 }}" style="text-decoration: none;color: #2a2a2a;">我的作业</router-link>
        </a>
        <a slot="taskImg">
          <router-link v-if="identity==='teacher'" :to="{name:'tctask',query: { course: item.coursename,status:0 }}"><img src="@/assets/1.png"></router-link>
          <router-link v-if="identity==='student'" :to="{name:'task1',query: { course: item.coursename,status:0 }}"><img src="@/assets/1.png"></router-link>
        </a>
        <a slot="task">
          <router-link v-if="identity==='teacher'" :to="{name:'tctask',query: { course: item.coursename,status:0 }}" style="text-decoration: none;color: #2a2a2a;">互评任务</router-link>
          <router-link v-if="identity==='student'" :to="{name:'task1',query: { course: item.coursename,status:0 }}" style="text-decoration: none;color: #2a2a2a;">互评任务</router-link>
        </a>
...
export default {
  ...
  created(){
    axios.post('/my/course/getcourse',{status:0,username:this.$route.params.id}).then(res=>{
