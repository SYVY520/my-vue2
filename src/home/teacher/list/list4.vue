<template>
  <div>
    ...
    <lists v-for="item in pageList" :key="item.id">
      <a slot="name">
        <router-link :to="{name:'tcdetails0',query:{task:item.tasknameut,username: item.username }}">{{item.name}}</router-link>
      </a>
...
export default {
  ...
  created(){
    axios.post('/my/course/getstudent3',{taskname:this.$route.query.task}).then(res=>{
