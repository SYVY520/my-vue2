<template>
  <div>
    ...
    <lists v-for="item in pageList" :key="item.id">
      <a slot="name">
        <router-link :to="{name:'tcdetails',query: {homework:item.homeworknameuh,username: item.username }}">{{item.name}}</router-link>
      </a>
...
export default {
  ...
  created(){
    axios.post('/my/course/getstudent1',{homeworkname:this.$route.query.homework}).then(res=>{
