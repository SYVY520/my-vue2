<template>
  <headers>
    <h3 slot="title"><img id="left" src="@/assets/left.png" @click="left">作业详情</h3>
    <div id="detailsBox">
  ...
  methods:{
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('/my/course/updatehomework',{username:this.$route.query.username,homeworkname:this.$route.query.homework,commentsuh:this.inputFooter,score:this.numberValidateForm.age}).then(res=>{
            console.log(res.data.data)
            if(res.data.status===0){
              this.$message({
                type: 'success',
