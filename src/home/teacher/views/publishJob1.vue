...
<div id="publishJobBox">
  <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
    <el-form-item label="作业名称" prop="name"><el-input v-model="ruleForm.name" disabled></el-input></el-form-item>
    ...
    <el-form-item><el-button type="primary" @click="submitForm('ruleForm')" style="margin-top: 205px;">修改作业</el-button>
...
export default {
  ...
  created(){
    axios.post('/my/course/homework',{homeworkname:this.ruleForm.name}).then(res=>{
      if(res.data.status===0) {
          this.ruleForm.date1=res.data.data[0].data
          this.ruleForm.date2=res.data.data[0].time
          this.ruleForm.desc=res.data.data[0].topic
      }
  ...
  methods: {
    submitGo(){
      let number1 = false;
      this.$emit('cancel',number1);
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('/my/course/xghomework',{homeworkname:this.ruleForm.name,course:this.$route.query.course,data:this.ruleForm.date1,time:this.ruleForm.date2,topic:this.ruleForm.desc}).then(res=>{
